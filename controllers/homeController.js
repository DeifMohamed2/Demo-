...
const public_Register_post = async (req, res) => {
  ...
  if (Grade ==="Grade1") {
    await User.findOne({Grade:Grade,Code:754909}).then((result)=>{
      quizesInfo = result.quizesInfo
      videosInfo = result.videosInfo
      
    })
  }else if(Grade ==="Grade2"){
    await User.findOne({Grade:Grade,Code:914777}).then((result)=>{
      quizesInfo = result.quizesInfo
      videosInfo = result.videosInfo
    })
  }else if(Grade ==="Grade3"){
    await User.findOne({Grade:Grade,Code:947484}).then((result)=>{
      quizesInfo = result.quizesInfo
      videosInfo = result.videosInfo
    })
  }
